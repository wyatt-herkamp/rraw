...
impl<'de> Deserialize<'de> for GenericResponse<RedditType> {
    fn deserialize<D>(deserializer: D) -> Result<GenericResponse<RedditType>, D::Error> where D: Deserializer<'de>, {
        let value: serde_json::Value = serde::Deserialize::deserialize(deserializer).unwrap();

        let x = value["kind"].as_str().unwrap();
        match x {
            "t1" => { Ok(GenericResponse::<RedditType>::new(RedditType::Comment((serde_json::from_value(value["data"].clone()).unwrap())))) }
            "t2" => { Ok(GenericResponse::<RedditType>::new(RedditType::Account((serde_json::from_value(value["data"].clone()).unwrap())))) }
            "t3" => { Ok(GenericResponse::<RedditType>::new(RedditType::Link((serde_json::from_value(value["data"].clone()).unwrap())))) }
            //"t4" => { Ok(GenericResponse::<RedditType>::new(RedditType::Comment(serde_json::from_str(value["data"].as_str().unwrap()).unwrap()))) }
            "t5" => { Ok(GenericResponse::<RedditType>::new(RedditType::Subreddit(serde_json::from_value(value["data"].clone()).unwrap()))) }
...
impl<'de,T> Deserialize<'de> for GenericListing<T> {
    fn deserialize<D>(deserializer: D) -> Result<GenericListing<T>, D::Error> where D: Deserializer<'de>, {
        let value: serde_json::Value = serde::Deserialize::deserialize(deserializer).unwrap();
