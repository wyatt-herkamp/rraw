use reqwest::{Client, Request, RequestBuilder, Body};
use std::collections::HashMap;
use reqwest::header::{HeaderName, AUTHORIZATION, USER_AGENT, CONTENT_TYPE, HeaderMap, HeaderValue};
...
impl PasswordAuthenticator {
    pub fn new(client_id: &str, client_secret: &str, username: &str, password: &str) -> Arc<Mutex<Box<dyn Auth + Send>>> {
...
impl Auth for PasswordAuthenticator {
    async fn login(&mut self, client: &Client, user_agent: &str) -> Result<bool, APIError> {
        ...
        header.insert(AUTHORIZATION, HeaderValue::from_str(&*format!("Basic {}", base64::encode(format!("{}:{}", self.client_id.to_owned(), self.client_secret.to_owned())))).unwrap());
        header.insert(USER_AGENT, HeaderValue::from_str(user_agent).unwrap());
        header.insert(CONTENT_TYPE, HeaderValue::from_str("application/x-www-form-urlencoded").unwrap());
        let result2 = client.post(url).body(Body::from(body)).headers(header).send().await;
    ...
    fn headers(&self, headers: &mut HeaderMap) {
        headers.insert(AUTHORIZATION, HeaderValue::from_str(&*format!("Bearer {}", self.token.to_owned().unwrap())).unwrap());
    }
